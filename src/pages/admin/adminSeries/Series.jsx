...
export function Series() {
  ...
  const handleSaveFromModal = async (formData, isEditing) => {
    try {
      ...
      console.error(error);
      showNotification('Error saving series.', 'error');
    }
  };

  const handleDelete = async (id, name) => {
    if (window.confirm(`Delete series "${name}"? All linked collections may be affected.`)) {
      try {
        await serieService.deleteSeries(id);
        setSeriesList((prev) => prev.filter((s) => s.id !== id));
      } catch {
        showNotification(
          'Error deleting. Make sure there are no collections linked to this series.',
          'error'
        );
      }
    }
