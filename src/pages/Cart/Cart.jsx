...
export function Cart() {
  ...
  const handleCheckout = async () => {
    if (!isAuthenticated) {
      showNotification('You need to be logged in to complete the purchase.', 'warning');
      navigate('/login');
      return;
    }
    ...
    try {
      ...
      console.log('Sending order (JSON):', payload);
      ...
      showNotification('Error processing order. Check console for details.', 'error');
    } finally {
  ...
  return (
    <main>
      ...
      <Container
        maxWidth="lg"
        sx={{ py: 5, minHeight: '80vh', display: 'flex', flexDirection: 'column' }}
      >
        <PageName>My Cart</PageName>
        ...
        {cartItems.length === 0 ? (
          ...
          >
            ...
            <Button variant="contained" sx={{ mt: 3 }} onClick={() => navigate('/marketplace')}>
              See products
            </Button>
          </Box>
        ) : (
              ...
              {cartItems.map((item) => (
                ...
                >
                  ...
                  <Box
                    display="flex"
                    alignItems="center"
                    sx={{ border: '1px solid #ddd', borderRadius: 1, mr: 2 }}
                  >
                    <IconButton
                      size="small"
